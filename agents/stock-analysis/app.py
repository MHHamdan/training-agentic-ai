...
class BasicStockAnalyzer:
    ...
    def get_stock_data(self, ticker):
        ...
        try:
            ...
            if hist.empty:
                st.error(f"No data found for ticker: {ticker}. Please verify the symbol is correct.")
                return None
            ...
            try:
                ...
                st.warning(f"Some company information unavailable: {str(info_error)}")
                info = {}
            
            # Calculate price data from historical data
            ...
            volume = hist['Volume'].iloc[-1] if not hist['Volume'].empty else info.get('volume', 0)
            ...
            if "No data found" in str(e) or "not found" in str(e).lower():
                st.info(f"""
                **Troubleshooting suggestions for {ticker}:**
                - Verify the ticker symbol is correct
                - Try common variations (e.g., AAPL for Apple)
                - Check if it's traded on major exchanges (NYSE, NASDAQ)
                - Some symbols may require suffixes (e.g., .TO for Toronto)
                """)
...
def main():
    ...
    with st.sidebar:
        ...
        if alpha_key:
            ...
            st.warning("⚠️ Alpha Vantage API (Optional)")
            
        if finnhub_key:
            st.success("✅ Finnhub API")
        else:
            st.warning("⚠️ Finnhub API (Optional)")
